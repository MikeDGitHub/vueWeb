...
import qs from "qs";
import config from '/Users/wannian/iceworks-workspace/Demo/vue.config.js'
...
export default {
  ...
  actions: {
    ...
    d2adminLogin({ state, commit, rootState }, { vm, username, password }) {
      ...
      vm.$post(vm.oauthService + 'connect/token', data)
        .then(res => {
          console.log(res);
          util.cookies.set('token', res.access_token)
          util.cookies.set('refresh_token', res.refresh_token)
          vm.$fetch(vm.account + 'getIdentity')
            .then((response) => {
              //设置 vuex 用户信息
              commit('d2adminUserInfoSet', {
                name: response.json.loginName
              })
              util.cookies.set('name', response.json.loginName)
              //用户登陆后从数据库加载一系列的设置
        ...
        .catch(err => {
          console.log(err);
          vm.$message.error(err.response.data.error_description)
        })
    },
    /**
     * 注销用户并返回登陆页面
  ...
  mutations: {
    ...
    d2adminUserInfoLoad() {
      this.commit('d2adminUtilDb2VuexByUuid', {
        key: 'userInfo',
        defaultValue: {
          name: util.cookies.get('name')//'请重新登陆'
        }
      })
    },
    /**
