    ...
    <el-table
      ...
      <template slot-scope="scope">
        <el-button class="fa fa-edit"
          @click="handleEdit(scope.$index, scope.row.valveId,scope.row.attachmentList)"></el-button>
      </template>
    </el-table-column>
    </el-table>
...
<el-collapse v-model="activeName" accordion>
  <el-collapse-item title="阀门信息" name="1">
     <el-form-item label="ID" prop="valveId">
    <el-input v-model="ruleForm.valveId" disabled></el-input>
  </el-form-item>
    <el-form-item label="编号" prop="valveCode">
    <el-input v-model="ruleForm.valveCode"></el-input>
  </el-form-item>
    <el-form-item label="名称" prop="valveName">
    <el-input v-model="ruleForm.valveName"></el-input>
  </el-form-item>
...
export default {
  ...
  methods: {
    ...
    handleEdit(index, row, attachmentList) {
      ...
      this.$post(this.valve + "queryInfo", { Id: row })
        .then(res => {
    ...
    resetForm() {
      this.$post(this.valve + "queryInfo", { Id: this.ruleForm.valveId })
        .then(res => {
          this.ruleForm = res.json;
          this.dialogFormVisible = true;
          console.log(res);
    ...
    querytable(pageIndex,address='') {
      this.loading = true;
      this.$post(this.valve + "queryList", {
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          console.log(valve);
          this.$post(this.valve + "update", valve)
            .then(res => {
              if (res.msg === "") {
                this.$message({
                  message: "更新成功",
