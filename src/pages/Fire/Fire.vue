    ...
    <el-table
      ...
      <template slot-scope="scope">
        <el-button class="fa fa-edit"
          @click="handleEdit(scope.$index, scope.row.fireFightingId,scope.row.attachmentList)"></el-button>
      </template>
    </el-table-column>
    </el-table>
...
<el-collapse v-model="activeName" accordion>
  <el-collapse-item title="消防信息" name="1">
     <el-form-item label="ID" prop="fireFightingId">
    <el-input v-model="ruleForm.fireFightingId" disabled></el-input>
  </el-form-item>
    <el-form-item label="编号" prop="fireFightingCode">
    <el-input v-model="ruleForm.fireFightingCode"></el-input>
  </el-form-item>
    <el-form-item label="名称" prop="fireFightingName">
    <el-input v-model="ruleForm.fireFightingName"></el-input>
  </el-form-item>
 ...
 data() {
  ...
  methods: {
    ...
    handleEdit(index, row, attachmentList) {
      ...
      this.$post(this.fireFighting + "queryInfo", { Id: row })
        .then(res => {
    ...
    resetForm() {
      this.$post(this.fireFighting + "queryInfo", { Id: this.ruleForm.fireFightingId })
        .then(res => {
          this.ruleForm = res.json;
          this.dialogFormVisible = true;
          console.log(res);
    ...
    querytable(pageIndex,address='') {
      this.loading = true;
      this.$post(this.fireFighting + "queryList", {
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          let fireFighting = {
            fireFighting: this.ruleForm,
            list: this.attachmentList,
            user: this.userInfo
          };
          console.log(fireFighting);
          this.$post(this.fireFighting + "update", fireFighting)
            .then(res => {
              if (res.msg === "") {
                this.$message({
                  message: "更新成功",
