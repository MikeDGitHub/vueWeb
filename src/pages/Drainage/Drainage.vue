    ...
    <el-table
      ...
      <template slot-scope="scope">
        <el-button class="fa fa-edit"
          @click="handleEdit(scope.$index, scope.row.drainageId,scope.row.attachmentList)"></el-button>
      </template>
    </el-table-column>
    </el-table>
...
<el-collapse v-model="activeName" accordion>
  <el-collapse-item title="排水信息" name="1">
     <el-form-item label="ID" prop="drainageId">
    <el-input v-model="ruleForm.drainageId" disabled></el-input>
  </el-form-item>
    <el-form-item label="编号" prop="drainageCode">
    <el-input v-model="ruleForm.drainageCode"></el-input>
  </el-form-item>
    <el-form-item label="名称" prop="drainageName">
    <el-input v-model="ruleForm.drainageName"></el-input>
  </el-form-item>
...
export default {
  ...
  methods: {
    ...
    handleEdit(index, row, attachmentList) {
      ...
      this.$post(this.drainage + "queryInfo", { Id: row })
        .then(res => {
    ...
    resetForm() {
      this.$post(this.drainage + "queryInfo", { Id: this.ruleForm.drainageId })
        .then(res => {
          this.ruleForm = res.json;
          this.dialogFormVisible = true;
          console.log(res);
    ...
    querytable(pageIndex,address='') {
      this.loading = true;
      this.$post(this.drainage + "queryList", {
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          // this.ruleForm.waterMeterName = this.ruleForm.drainageName;
          // this.ruleForm.waterMeterCode = this.ruleForm.drainageCode;
          // this.ruleForm.DrainageId = this.ruleForm.drainageId;
          let drainage = {
            drainage: this.ruleForm,
            list: this.attachmentList,
            user: this.userInfo
          };
          console.log(drainage);
          this.$post(this.drainage + "update", drainage)
            .then(res => {
              if (res.msg === "") {
                this.$message({
                  message: "更新成功",
