    ...
    <el-table
      ...
      <template slot-scope="scope">
        <el-button class="fa fa-edit"
          @click="handleEdit(scope.$index, scope.row.pipeLineId,scope.row.attachmentList,scope.row.trackInfo)"></el-button>
      </template>
    </el-table-column>
    </el-table>
...
<el-collapse v-model="activeName" accordion>
  <el-collapse-item title="管线信息" name="1">
     <el-form-item label="ID" prop="pipeLineId">
    <el-input v-model="ruleForm.pipeLineId" disabled></el-input>
  </el-form-item>
    <el-form-item label="编号" prop="pipeLineCode">
    <el-input v-model="ruleForm.pipeLineCode"></el-input>
  </el-form-item>
    <el-form-item label="名称" prop="pipeLineName">
    <el-input v-model="ruleForm.pipeLineName"></el-input>
  </el-form-item>
...
export default {
  ...
  methods: {
    ...
    handleEdit(index, row, attachmentList,trackInfo) {
      debugger;
      this.button = "保存";
      this.ruleForm.trackInfo=trackInfo;
      ...
      this.$post(this.pipeLine + "queryInfo", { Id: row })
        .then(res => {
    ...
    resetForm() {
      this.$post(this.pipeLine + "queryInfo", { Id: this.ruleForm.pipeLineId })
        .then(res => {
          this.ruleForm = res.json;
          this.dialogFormVisible = true;
          console.log(res);
    ...
    querytable(pageIndex,address='') {
      this.loading = true;
      this.$post(this.pipeLine + "queryList", {
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          let pipeLine = {
            pipeLine: this.ruleForm,
            list: this.attachmentList,
            user: this.userInfo,
            trackInfo:this.ruleForm.trackInfo
          };
          console.log(pipeLine);
          this.$post(this.pipeLine + "update", pipeLine)
            .then(res => {
              if (res.msg === "") {
                this.$message({
                  message: "更新成功",
