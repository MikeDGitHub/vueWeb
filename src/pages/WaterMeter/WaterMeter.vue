    ...
    <el-table
      ...
      <template slot-scope="scope">
        <el-button class="fa fa-edit"
          @click="handleEdit(scope.$index, scope.row.waterId,scope.row.attachmentList)"></el-button>
      </template>
    </el-table-column>
    </el-table>
...
<el-collapse v-model="activeName" accordion>
  <el-collapse-item title="水表信息" name="1">
     <el-form-item label="ID" prop="waterId">
    <el-input v-model="ruleForm.waterId" disabled></el-input>
  </el-form-item>
    <el-form-item label="编号" prop="waterCode">
    <el-input v-model="ruleForm.waterCode"></el-input>
  </el-form-item>
    <el-form-item label="名称" prop="waterName">
    <el-input v-model="ruleForm.waterName"></el-input>
  </el-form-item>
...
export default {
  ...
  methods: {
    ...
    querytable(pageIndex, address = "") {
      this.loading = true;
      this.$post(
        this.water + "queryList",
        { PageSize: this.pageSize, PageIndex: parseInt(pageIndex) - 1,Address: address },
      )
        .then(res => {
    ...
    handleEdit(index, row, attachmentList) {
      ...
      this.$post(this.water + "queryInfo", { Id: row })
        .then(res => {
    ...
    resetForm() {
      this.$post(this.water + "queryInfo", { Id: this.ruleForm.waterId })
        .then(res => {
          this.ruleForm = res.json;
          this.dialogFormVisible = true;
          console.log(res);
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.ruleForm.waterMeterName = this.ruleForm.waterName;
          this.ruleForm.waterMeterCode = this.ruleForm.waterCode;
          this.ruleForm.waterMeterId = this.ruleForm.waterId;
          let water = {
            waterMeter: this.ruleForm,
            list: this.attachmentList,
            user: this.userInfo
          };
          console.log(water);
          this.$post(this.water + "update", water)
            .then(res => {
              if (res.msg === "") {
                this.$message({
                  message: "更新成功",
