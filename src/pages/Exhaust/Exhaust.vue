    ...
    <el-table
      ...
      <template slot-scope="scope">
        <el-button class="fa fa-edit"
          @click="handleEdit(scope.$index, scope.row.exhaustId,scope.row.attachmentList)"></el-button>
      </template>
    </el-table-column>
    </el-table>
...
<el-collapse v-model="activeName" accordion>
  <el-collapse-item title="排气信息" name="1">
     <el-form-item label="ID" prop="exhaustId">
    <el-input v-model="ruleForm.exhaustId" disabled></el-input>
  </el-form-item>
    <el-form-item label="编号" prop="exhaustCode">
    <el-input v-model="ruleForm.exhaustCode"></el-input>
  </el-form-item>
    <el-form-item label="名称" prop="exhaustName">
    <el-input v-model="ruleForm.exhaustName"></el-input>
  </el-form-item>
 ...
 components: {},
  ...
  methods: {
    ...
    handleEdit(index, row, attachmentList) {
      ...
      this.$post(this.exhaust + "queryInfo", { Id: row })
        .then(res => {
    ...
    resetForm() {
      this.$post(this.exhaust + "queryInfo", { Id: this.ruleForm.exhaustId })
        .then(res => {
          this.ruleForm = res.json;
          this.dialogFormVisible = true;
          console.log(res);
    ...
    querytable(pageIndex,address='') {
      this.loading = true;
      this.$post(this.exhaust + "queryList", {
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          let exhaust = {
            exhaust: this.ruleForm,
            list: this.attachmentList,
            user: this.userInfo
          };
          console.log(exhaust);
          this.$post(this.exhaust + "update", exhaust)
            .then(res => {
              if (res.msg === "") {
                this.$message({
                  message: "更新成功",
